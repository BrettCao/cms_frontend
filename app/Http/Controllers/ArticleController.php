...
class ArticleController extends Controller
{
    public function getIndex()
    {
        ...
        $this->layoutData['content'] = View::make('admin.pages.articles', array('pages' => Page::getArticleView(0), 'add_page' => Auth::action('pages.add'), 'page_states' => Auth::user()->getPageStates(), 'max' => Page::at_limit(), 'rootPageIds' => '', 'groups_exist' => PageGroup::count(),'page'=>$articles));
    ...
    public function getAdd($parentPageId = 0, $groupId = 0)
    {
        ...
        list($tab_headers[0], $tab_contents[0]) = $page->tabInfoArticle();
        ksort($tab_headers);


        $tab_data = [
            'headers' => View::make('admin.tabs.article_header', ['tabs' => $tab_headers])->render(),
            'contents' => View::make('admin.tabs.article_content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => true, 'publishing' => $publishingOn, 'can_publish' => $cabPublish, 'page' => $page])->render()
        ];
//        dd($tab_data);
        $this->layoutData['content'] = View::make('admin.pages.article_add', [
            'page' => $page,
            'item_name' => $item_name,
            'tab' => $tab_data
        ]);
    ...
    public function postAdd($pageId = 0, $groupId = 0)
    {
        ...
        if (!$page->savePostData($page_version, $page_info, $page_info_lang, $page_groups, $page_info_other)) {
            ...
            return \redirect()->route('coaster.admin.article');
        }
    }
    ...
    public function getEdit($pageId, $versionId = 0)
    {
        ...
        $tab_data = [
            ...
            'contents' => View::make('admin.tabs.article_edit_content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => false, 'publishing' => $publishingOn, 'can_publish' => $auth['can_publish'], 'page' => $page])->render()
        ];
