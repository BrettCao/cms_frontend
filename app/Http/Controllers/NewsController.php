...
class NewsController extends Controller
{
    public function getIndex()
    {
        ...
        $childPages = Page::where('template',1)->whereIn('parent',$categories)->select(['id','parent'])->paginate(15);
        $this->layoutData['content'] = View::make('admin.pages.news', array('pages' => Page::getNewsView(0), 'add_page' => Auth::action('pages.add'), 'page_states' => Auth::user()->getPageStates(), 'max' => Page::at_limit(), 'rootPageIds' => '', 'groups_exist' => PageGroup::count(),'page'=>$childPages));
    ...
    public function getAdd($parentPageId = 0, $groupId = 0)
    {
        ...
        list($tab_headers[0], $tab_contents[0]) = $page->tabInfoNews();
        ksort($tab_headers);


        $tab_data = [
            'headers' => View::make('admin.tabs.article_header', ['tabs' => $tab_headers])->render(),
            'contents' => View::make('admin.tabs.news_content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => true, 'publishing' => $publishingOn, 'can_publish' => $cabPublish, 'page' => $page])->render()
        ];
//        dd($tab_data);
        $this->layoutData['content'] = View::make('admin.pages.news_add', [
            'page' => $page,
            'item_name' => $item_name,
            'tab' => $tab_data
        ]);
    ...
    public function postAdd($pageId = 0, $groupId = 0)
    {
        ...
        if (!$page->savePostData($page_version, $page_info, $page_info_lang, $page_groups, $page_info_other)) {
            ...
            return \redirect()->route('coaster.admin.news');
        }
    }
    ...
    public function getEdit($pageId, $versionId = 0)
    {
        ...
        $tab_data = [
            ...
            'contents' => View::make('admin.tabs.news_edit_content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => false, 'publishing' => $publishingOn, 'can_publish' => $auth['can_publish'], 'page' => $page])->render()
        ];
