...
class CategoryController extends Controller
{
    ...
    {
        $this->layoutData['content'] = View::make('admin.pages.pages', array('pages' => Page::getCategoryView(0), 'add_page' => Auth::action('pages.add'), 'page_states' => Auth::user()->getPageStates(), 'max' => Page::at_limit(), 'rootPageIds' => '', 'groups_exist' => PageGroup::count()));
    ...
    public function getAdd($parentPageId = 0, $groupId = 0)
    {
        ...
        $tab_headers = [];
        $tab_contents = [];
        list($tab_headers[0], $tab_contents[0]) = $page->tabInfoCategory();

        $tab_data = [
            'headers' => View::make('admin.tabs.header', ['tabs' => $tab_headers])->render(),
            'contents' => View::make('admin.tabs.content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => true, 'publishing' => $publishingOn, 'can_publish' => $cabPublish, 'page' => $page])->render()
        ];

        $this->layoutData['content'] = View::make('admin.pages.add', [
            'page' => $page,
            'item_name' => $item_name,
            'tab' => $tab_data
        ]);
    ...
    public function postAdd($pageId = 0, $groupId = 0)
    {
        ...
        if (!$page->savePostData($page_version, $page_info, $page_info_lang, $page_groups, $page_info_other)) {
            ...
            return \redirect()->route('coaster.admin.category');
        }
    }
    ...
    public function getEdit($pageId, $versionId = 0)
    {
        ...
        // load blocks content
        if ($page->link == 0) {
            $blocks = ThemeTemplate::templateBlocks(1, $page->template);
            $blocks_content = PageBlock::preloadPage($pageId, $versionData['editing']);
            list($tab_headers, $tab_contents) = Block::getCategoryTabs($blocks, $blocks_content, $page->id, $versionId);
        } else {
            $tab_headers = [];
            $tab_contents = [];
        }
        ...
        if ($publishingOn && count($tab_headers) > 1) {
            ...
            $tab_contents[-1] = View::make('admin.partials.tabs.versions.main', ['content' => PageVersion::version_table($page->id)])->render();
        ...
        $tab_data = [
            'headers' => View::make('admin.tabs.header', ['tabs' => $tab_headers])->render(),
            'contents' => View::make('admin.tabs.content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => false, 'publishing' => $publishingOn, 'can_publish' => $auth['can_publish'], 'page' => $page])->render()
        ];
