...
class PrintController extends Controller
{
    public function getIndex()
    {
        ...
        $this->layoutData['content'] = View::make('admin.pages.print', array('pages' => Page::getPrintFrontierView(0), 'add_page' => Auth::action('pages.add'), 'page_states' => Auth::user()->getPageStates(), 'max' => Page::at_limit(), 'rootPageIds' => '', 'groups_exist' => PageGroup::count(),'page'=>$childPages));
    ...
    public function getAdd($parentPageId = 0, $groupId = 0)
    {
        ...
        if ($page->link == 0) {
            ...
            $blocks_content = PageBlock::preloadPage($page->id, 2);
        ...
        list($tab_headers[0], $tab_contents[0]) = $page->tabInfoPrint();
        ksort($tab_headers);
...
//        dd($tab_data);
        $this->layoutData['content'] = View::make('admin.pages.print_add', [
            'page' => $page,
            'item_name' => $item_name,
            'tab' => $tab_data
        ]);
    ...
    public function postAdd($pageId = 0, $groupId = 0)
    {
        ...
        $page_info['template'] = ['exists'=>1,'select'=>1];
        ...
        if (!$page->savePostData($page_version, $page_info, $page_info_lang, $page_groups, $page_info_other)) {
            ...
            $l_page = $existingPage->page_langs()->first();
            if($l_page){
                if(Request::input('e_title')){
                    $l_page->e_title = Request::input('e_title');
                }
                if(Request::input('competent')){
                    $l_page->competent = Request::input('competent');
                }
                if(Request::input('unit')){
                    $l_page->unit = Request::input('unit');
                }
                if(Request::input('issn')){
                    $l_page->issn = Request::input('issn');
                }
                if(Request::input('issue')){
                    $l_page->issue = Request::input('issue');
                }
                if(Request::input('press')){
                    $l_page->press = Request::input('press');
                }
                if(Request::input('num')){
                    $l_page->num = Request::input('num');
                }
                if(Request::input('cycle')){
                    $l_page->cycle = Request::input('cycle');
                }
                if(Request::input('start_publish_date')){
                    $l_page->start_publish_date = Request::input('start_publish_date');
                }
                $l_page->save();
            }
            return \redirect()->route('coaster.admin.print');
        }
    }
    ...
    public function getEdit($pageId, $versionId = 0)
    {
        ...
        $tab_data = [
            ...
            'contents' => View::make('admin.tabs.journal_edit_content', ['tabs' => $tab_contents, 'item' => $item_name, 'new_page' => false, 'publishing' => $publishingOn, 'can_publish' => $auth['can_publish'], 'page' => $page])->render()
        ];
    ...
    public function postDelete($pageId)
    {
        if ($page = Page::find($pageId)) {
            ...
            return redirect()->route("coaster.admin.journal");
        }
